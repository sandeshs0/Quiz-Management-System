...
exitBut = PhotoImage(
    file=r'C:\sandesh\Python\tkinterrr\FirstSem Project - Copy\Khichadi\exitbtn.png')
...
# Title and Subject
tit = Label(newQuizApp, text=title, bg="White",
...
correctOptCombo['values'] = (1, 2, 3, 4)
...
def fetchqns():
    ...
    tree = ttk.Treeview(newQuizApp, style="mystyle.Treeview", columns=(
        "ID", "Questions"), show="headings")
    tree.column("ID", width=50, anchor="center")
    tree.column("Questions", width=500, anchor="center")
    tree.heading("ID", text="ID")
    tree.heading("Questions", text="Questions")
    tree.place(x=700, y=110)
...
# Add Question Button
btn = Button(container, image=addBtn, bg="#F9ECEC", bd=0,
             borderwidth=0, activebackground="#F9ECEC", command=addQns)
btn.place(x=280, y=415)

# Exit Button
exitBtn = Button(newQuizApp, image=exitBut, bg="white", bd=0,
                 borderwidth=0, activebackground="white", command=exitfun)
