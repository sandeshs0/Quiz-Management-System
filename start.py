...
def switchtoAdminLogin():
    window.destroy()
    runpy.run_path(
        "C:\\sandesh\\Python\\tkinterrr\\FirstSem Project - Copy\\Khichadi\\AdminLogin.py")
    # import AdminLogin


def switchtoStudentLogin():
    window.destroy()
    runpy.run_path(
        "C:\\sandesh\\Python\\tkinterrr\\FirstSem Project - Copy\\Khichadi\\studentLogin.py")
    # import studentLogin
...
illuslab = Label(window, text='Welcome to EzyQzy! \n  The Ultimate Quiz Management System.',
...
def continueAs():
    ...
    # Button 1
    asAdmin = Button(pane, image=anadmin, command=switchtoAdminLogin,
                     borderwidth=0, bg="white", activebackground="white")
    asAdmin.pack(side=LEFT, expand=True, fill=BOTH)

    # Button 2
    asStudent = Button(pane, image=astudent, command=switchtoStudentLogin,
                       borderwidth=0, bg="white", activebackground="white")
